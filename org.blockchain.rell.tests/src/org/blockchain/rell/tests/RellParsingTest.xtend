...
// check tuple return from query !ERROR (3:11) Return type of query 'q' is not GTX-conversible: (lastName:text,text)
//	@Test
...
// list ''+ from at expression !ERROR (3:10) Return type of query 'q' is not GTX-conversible: (lastName:text,text)
//	@Test
...
// check 'in' set !ERROR (8:50) Operator '[]' undefined for type set<integer>
//	@Test
	def void testSetMethods() {
		...
		query q6() { val x = set<integer>([123]); x[0] = 456; return ''+x; } // ERROR IN THIS PLACE
		query q7() { val x = set<integer?>([123,456,null]); x.removeAll(set<integer>([123])); return ''+x; }
		query q8() { val x = set<integer>([123,456]); return x.containsAll(set<integer>([123])); }''')
	}
